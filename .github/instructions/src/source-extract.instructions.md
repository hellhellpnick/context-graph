---
description: "Mirror — `src/source-extract.ts`"
applyTo: "src/source-extract.ts"
priority: "P1"
last_updated: "2026-05-18"
---

## When to Read
- editing or refactoring `source-extract.ts`

## Overview
- `src/source-extract.ts` (1001 lines · 28 top-level symbols) — Shared helpers for Vue / PHP / Python / Go so scanner + deterministic graph use the same shapes.

## Graph
```mermaid
graph LR
  source_extract[source-extract]
  source_extract --> typescript["typescript"]
  source_extract --> ENV{{"env / config"}}
```
