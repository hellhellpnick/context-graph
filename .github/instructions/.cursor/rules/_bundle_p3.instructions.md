---
description: "Mirror — `.cursor/rules/` (4 files, part 3/20)"
...
## When to Read
- editing or refactoring `ctxgraph--cursor-rules-bundle-p3.mdc`
...
## Overview
- `.cursor/rules/ctxgraph--cursor-rules-bundle-p3.mdc` (54 lines · 4 top-level symbols) — # When to Read
- `.cursor/rules/ctxgraph--cursor-rules-bundle-p4.mdc` (48 lines · 3 top-level symbols) — # When to Read
- `.cursor/rules/ctxgraph--cursor-rules-bundle-p5.mdc` (53 lines · 7 top-level symbols) — # When to Read
- `.cursor/rules/ctxgraph--cursor-rules-bundle-p6.mdc` (111 lines · 32 top-level symbols) — # When to Read
...
export function registerActualizeCommand(program: Command): void { /* prompt template (~102 lines) */ }
// ── .cursor/rules/ctxgraph--src-cli-commands-agents.mdc ──
// ── src/cli/commands/agents.ts ──
export function registerAgentsCommand(program: Command): void { /* prompt template (~77 lines) */ }
// ── .cursor/rules/ctxgraph--src-cli-commands-build.mdc ──
// ── src/cli/commands/build.ts ──
export function registerBuildCommand(program: Command): void { /* prompt template (~244 lines) */ }
// ── .cursor/rules/ctxgraph--src-cli-commands-hook-check.mdc ──
// ── src/cli/commands/hook-check.ts ──
export function registerHookCheckCommand(program: Command): void { /* prompt template (~69 lines) */ }
...
export const OUTPUT_FORMAT_INSTRUCTION = ` --- ## MANDATORY OUTPUT FORMAT You MUST wrap every output file in these exact delimiters. No prose outside the blocks. <<<FILE: path/relative/to/project/root>>> file content … { /* prompt template (~23 lines) */ }
